...
// LSM algorith for improving the fringe rate and delays
  ...
  for (irow = 0; irow < nrows; ++irow) {
    ...
    for (pol = 0; pol < npol; ++pol) {
      for (chan = 0; chan < nchannels; ++chan) {
        ...
        flags(row_new, chan, pol) = flag_data(pos);
      }
    }
    if (irow % 10000 == 0) {
...
void filter_baseline(MeasurementSet &ms, flag_type &baseline_ok, uint tini, uint tend, uint n_bls, double threshold=0.3){
  uint row_ini = tini*n_bls;
  uint row_end = tend*n_bls;
  uint tskip = tend-tini;
  std::ostringstream query;
...
void first_guess(
  params_type &params, visibility_type &visibilities, flag_type &baseline_ok, time_type &time,
  frequency_type &freq, double f0_, uint nant, uint refant, uint nt, uint nf, uint nscale)
{
  ...
  for (iant = 0; iant < nant; ++iant) {
    ...
    xt::xarray<std::complex<float>> vis_slice = xt::view(
        visibilities,
    ...
    for (ti = 0; ti < nt_new; ++ti) {
      for (fi = 0; fi < nf_new; ++fi) {
        if (ti < nt && fi < nf) {
          ...
          F_tf[ti * nf_new + fi][0] = val.real();
          F_tf[ti * nf_new + fi][1] = val.imag();
        } else {
  ...
  // --- Rate (Time axis) ---
    ...
    params(iant, 1) = float(sgn * rate);
    params(iant, 2) = float(sgn * delay);

    std::cout << " Ant:   " << iant
              << "\tPhase: " << params(iant, 0)
              << "\tRate:  " << params(iant, 1)*1e3
              << "\tDelay: " << params(iant, 2)*1e9
              << std::endl;
    }
...
int main(void) {
  ...
  const std::string msin = "../Data/ILTJ125911.17+351954.5_143MHz_uv.dp3-concat";
  ...
  first_guess(prms, vis, baseline_ok, time, freq, f0_, nant, refant, nt_loc, nf, nscale);
