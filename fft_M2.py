...
t1   = data.query('ANTENNA1 == '+str(refant) +' AND ANTENNA2 == '+str(ant2))   # do row selection
...
vis_dat = t1.getcol('DATA')[tt1:tt2+1, ff1:ff2+1, n_pol]   # (channel, pol [RR, RL, LR, LL])
flg = t1.getcol('FLAG')[tt1:tt2+1, ff1:ff2+1, n_pol]   # (channel, pol [RR, RL, LR, LL])

####### FFT ######
vis_new = np.exp(1j*np.angle(vis_dat))
vis_new[flg] = 0.0
...
phase_0 = phase_FT[r_max_idx, tau_max_idx]
...
print(f"Estimated Phase (rads):      {phase_0}")

plt.figure(figsize=(12, 8))
plt.title(f"Vis for Baseline {refant}-{ant2}")
...
plt.title(f"Delay-Fringe Spectrum for Baseline {refant}-{ant2}")
